...
def kdtree_recursive_build(root, db, point_indices, axis, leaf_size):
    ...
    if len(point_indices) > leaf_size:
        ...
        # 作业1
        # 屏蔽开始
        # 如果当前点数大于 leaf_size,那么就应该分割新的下一级节点
        middle_left_index = math.ceil(point_indices_sorted.shape[0] * 0.5)
...
def kdtree_knn_search(root: Node, db: np.ndarray, result_set: KNNResultSet, query: np.ndarray):
    ...
    if query[root.axis] < root.value:
        ...
        if result_set.worstDist() > math.fabs(query[root.axis] - root.value):
